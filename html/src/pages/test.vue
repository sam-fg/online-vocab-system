    ...
    <div class="card border-info mb-3" style="max-width: 50rem;"> <!-- 2 -->
      ...
      <div class="card-body text-info">  <!-- 3 -->
        <div class="card-block">
            <h2 onmousedown='return false;' onselectstart='return false;'>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get("./api/assessment/" + vm.$root.assessmentName + "/size")
      .then(response => {
  ...
  methods: {
    ...
    checkAns() {
      ...
      axios
        .get(
          "./api/check/assessment/" +
            vm.$root.assessmentName +
            "/index/" +
            id +
            "?ans=" +
            ans
    ...
    getNextVocab() {
      const vm = this
      const wrongPercent =
        vm.$root.wrongVocabs.length / vm.$root.vocabids.length
